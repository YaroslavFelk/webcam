...
if($this->StartResultCache())
{
	...
	{
		$elemnts = array();
		if ($arParams['INCLUDE_ELEMENTS'] == 'Y') {
			...
			while ($arElement = $rsElements->GetNext()) {
				$elemnts[$arElement['IBLOCK_SECTION_ID']][] = $arElement;
			}
		}
		...
		while($arSection = $rsSections->GetNext())
		{
			...
			if ($elemnts[$arSection['ID']]) {
				foreach ($elemnts[$arSection['ID']] as $el) {
					$arResult["SECTIONS"][] = array(
						"ID" => $el["ID"],
						"DEPTH_LEVEL" => $arSection["DEPTH_LEVEL"]+1,
						"~NAME" => $el["NAME"],
						"~SECTION_PAGE_URL" => $el["DETAIL_PAGE_URL"],
					);
					$arResult["ELEMENT_LINKS"][$arSection["ID"]] = array($el["DETAIL_PAGE_URL"]);
				}
			}
...
//echo '<pre>',var_dump($aMenuLinksNew),'</pre>';
