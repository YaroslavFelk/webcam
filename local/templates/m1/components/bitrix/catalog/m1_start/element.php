...
<div class='row'>
	<div class='<?=($isSidebar ? 'col-md-9 col-sm-8' : 'col-xs-12')?>'>
		...
		if ($elementId > 0)
		{
            $arParams['DETAIL_LINK_IBLOCK'] = 'LINK_GOODS';
            $obCache = new CPHPCache();
            $recommendedCacheId = array('IBLOCK_ID' => $arParams['IBLOCK_ID']."_".$elementId);
            ...
            elseif ($obCache->StartDataCache())
            {
                ...
                if($ob = $res->GetNextElement()) {
                    $arProps = $ob->GetProperty($arParams['DETAIL_LINK_IBLOCK']);
                    if ($arProps['VALUE'])
                        $recommendedData = $arProps['VALUE'];
                ...
                if ($recommendedData) {
                    global $filter_link;
                    $filter_link = array('ID' => $recommendedData);
                }
            }
            if (!isset($arParams['DETAIL_LINK_IBLOCK']) || $arParams['DETAIL_LINK_IBLOCK'] != 'N' and $recommendedData)
            {
                ?>
