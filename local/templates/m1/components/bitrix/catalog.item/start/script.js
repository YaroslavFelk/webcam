(function (window){
	...
	window.JCCatalogItem.prototype = {
		init: function()
		{
			...
			if (this.visual.QUICK_SHOW_ID)
			{
				this.obQuickShowBtn = BX(this.visual.QUICK_SHOW_ID);
			}
...
//console.log(this.errorCode)
			if (this.errorCode === 0)
			{
				...
				if (this.obQuickShowBtn)
				{

					BX.bind(this.obQuickShowBtn, 'click', BX.proxy(this.quickShow, this));
		...
		quantitySet: function(index)
		{
			...
			if (this.errorCode === 0)
			{
				...
				if (this.obQuantityLimit.all)
				{
					...
					{
						BX.adjust(this.obQuantityLimit.value, {html: 'Нет в наличии'});
					...
					else
					{
						...
						let limit_icon = '<svg width="16" height="12" viewBox="0 0 16 12" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
							...
							'</svg>';
						BX.adjust(this.obQuantityLimit.value, {html: limit_icon+debuggerstrLimit});
		...
		{
			this.basketUrl = this.basketData.basketUrl;
		...
		add2Basket: function()
		{
			this.btn = jQuery("#"+this.obBuyBtn.id)
		...
		buyOneClick: function()
		{
			...
			//this.buildModal('one_click_'+this.selected_id, 'one_click')
		},

		quickShow: function()
		{
			this.btn = jQuery("#"+this.obQuickShowBtn.id)
			this.btn_text = this.btn.html();
			this.btn.html('<svg height="8" viewBox="0 0 120 30" xmlns="http://www.w3.org/2000/svg" fill="#313D50"> <circle cx="15" cy="15" r="15"> <animate attributeName="r" from="15" to="15" begin="0s" dur="0.8s" values="15;9;15" calcMode="linear" repeatCount="indefinite" /> <animate attributeName="fill-opacity" from="1" to="1" begin="0s" dur="0.8s" values="1;.5;1" calcMode="linear" repeatCount="indefinite" /> </circle> <circle cx="60" cy="15" r="9" fill-opacity="0.3"> <animate attributeName="r" from="9" to="9" begin="0s" dur="0.8s" values="9;15;9" calcMode="linear" repeatCount="indefinite" /> <animate attributeName="fill-opacity" from="0.5" to="0.5" begin="0s" dur="0.8s" values=".5;1;.5" calcMode="linear" repeatCount="indefinite" /> </circle> <circle cx="105" cy="15" r="15"> <animate attributeName="r" from="15" to="15" begin="0s" dur="0.8s" values="15;9;15" calcMode="linear" repeatCount="indefinite" /> <animate attributeName="fill-opacity" from="1" to="1" begin="0s" dur="0.8s" values="1;.5;1" calcMode="linear" repeatCount="indefinite" /> </circle> </svg>');

			var id = 'quick_show_'+this.product.id.toString();
			...
			jQuery.ajax({
				...
				success: function (data) {
					obj.buildModal(id, data)
					//btn.html(btn_text);
					obj.btn.html(obj.btn_text);
				}
			});
			//this.buildModal('quick_show_'+this.product.id.toString(), 'quick_show')
		},

		buildModal: function(id, content)
		{
			if (jQuery("#"+id).length == 0) {
				let text = '<div class="modal element_fast_view fade" id="' + id + '" tabindex="-1" role="dialog" aria-labelledby="' + id + 'Label">\n' +
		...
		sendToBasket: function()
		{
			...
			BX.ajax({
				method: 'POST',
				dataType: 'json',
				url: '/local/components/profitkit/basket.start/add.php?id='+this.product.id,
		...
		basketResult: function(arResult)
		{
			...
			setTimeout(function () {
				jQuery(".catalog_item").removeClass("active");
			}, 100);
