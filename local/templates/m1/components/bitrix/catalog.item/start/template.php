...
if (isset($arResult['ITEM']))
{
	...
	$itemIds = array(
        ...
        'BUY_ONE_CLICK' => $areaId.'_buy_one_click',
        'QUICK_SHOW' => $areaId.'_quick_show',
	...
	else
	{
        ...
        $price['PRINT_RATIO_PRICE'] = $item['PROPERTIES']['PRICE']['VALUE'];
        $price['PRINT_RATIO_BASE_PRICE'] = $item['PROPERTIES']['PRICEOLD']['VALUE'];
        if (\Bitrix\Main\Loader::IncludeModule("profitkit.shopstart"))
            $price = Profitkit\Shopstart\Shop::getProductPrice($item);
		...
		if (!$haveOffers)
		{
			$jsParams = array(
				...
				'VISUAL' => array(
                    ...
                    'BUY_ONE_CLICK_ID' => $itemIds['BUY_ONE_CLICK'],
                    'QUICK_SHOW_ID' => $itemIds['QUICK_SHOW'],
	...
	<?
   //echo '<pre>item=',print_r($item),'</pre>';
    //echo '<pre>jsParams=',print_r($jsParams),'</pre>';
